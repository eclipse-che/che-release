...
process_project() {
    ...
    local wf_result wf_status wf_conclusion wf_url
    ...
    wf_url=$(echo "$wf_result" | jq -r '.run_url // empty')

    debug_log "  workflow: status=$wf_status conclusion=$wf_conclusion"
    ...
    while [[ $i -lt $prs_count ]]; do
        ...
        if [[ "$found_count" -gt 0 ]]; then
            if [[ "$merged_count" -ge "$pr_expected_count" ]]; then
                ...
                local first_pr_number first_pr_url first_pr_state
                first_pr_number=$(echo "$found_prs" | jq -r '.[0].number')
                first_pr_url=$(echo "$found_prs" | jq -r '.[0].url')
                first_pr_state=$(echo "$found_prs" | jq -r '.[0].state')
    ...
    for pd in "${pr_details[@]+"${pr_details[@]}"}"; do
        ...
        if [[ "$pd_status" == "merged" ]]; then
            local pd_repo pd_title pd_num pd_url pd_required
            IFS='|' read -r pd_repo pd_title pd_num pd_url pd_required <<< "$pd_rest"
            echo "  ✓ PR #${pd_num}: ${pd_title} (merged)"
        elif [[ "$pd_status" == "open" ]]; then
            local pd_repo pd_title pd_num pd_url pd_branch pd_required
            IFS='|' read -r pd_repo pd_title pd_num pd_url pd_branch pd_required <<< "$pd_rest"
            ...
            local pd_repo pd_title pd_required
            IFS='|' read -r pd_repo pd_title pd_required <<< "$pd_rest"
            local pd_icon
...
for phase_key in $(get_phase_keys); do
    ...
    while [[ $local_i -lt $local_project_count ]]; do
        ...
        process_project "$local_project" "$VERSION" "$BRANCH"
        local_i=$((local_i + 1))
    done
done
